...
function sdpFilterCodec(kind, codec, realSdp) {
  ...
  lines.forEach((line) => {
    ...
    if (
      (isKind && !(skipMatch && !allowed.includes(parseInt(skipMatch[2])))) ||
      !isKind
    ) {
      sdp += `${line}\n`;
    }
...
export default function Monitor({ testSample }) {
  const [audioInputSourceLabel, setAudioInputSourceLabel] = useState(
    AUDIO_INPUT_SOURCES.microphone.label
  );
  ...
  useEffect(() => {
    ...
    if (isStreamingAudio) {
      ...
      openPeerConnection();
      if (audioInputSourceLabel === AUDIO_INPUT_SOURCES.microphone.label)
        startStreamingMicrophone();
      else if (audioInputSourceLabel === AUDIO_INPUT_SOURCES.testSample.label)
        startStreamingTestSample();
  ...
  const processorParamValues =
    (paramValues && paramValues[processorName]) || {};
  return (
    <div>
      <div>
      ...
      {processors && (
        <div>
          ...
          >
            {[NONE_PROCESSOR_LABEL, ...Object.keys(processors)].map(
              (processorName) => (
      ...
      {processorParams && (
        <div>
          <div>
            <button
              disabled={isEstimatingParams}
              onClick={startEstimatingParams}
            >
              Start estimating
            </button>
            <button
              disabled={!isEstimatingParams}
              onClick={stopEstimatingParams}
            >
              Stop estimating
            </button>
          </div>
      ...
      <div>
        <button
          ...
          onClick={() => setIsStreamingAudio(true)}
        >
          Start sending
        </button>
        <button
          disabled={!isStreamingAudio}
          onClick={() => setIsStreamingAudio(false)}
        >
          Stop sending
        </button>
        {audioStreamErrorMessage && (
          <div style={{ color: "#B33A3A", fontSize: "12px" }}>
            {audioStreamErrorMessage}.
          </div>
        )}
      </div>
